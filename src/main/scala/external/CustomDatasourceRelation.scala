...
class CustomDatasourceRelation(override val sqlContext : SQLContext, path : String, userSchema : StructType)
  ...
  override def buildScan(): RDD[Row] = {  //ATTENTION
    println("TableScan: buildScan called...")

    val schemaFields = schema.fields
    // Reading the file's content
    val rdd = sqlContext.sparkContext.wholeTextFiles(path).map(f => f._2) // second element of a tuple

    val rows = rdd.map(fileContent => {
      val lines = fileContent.split("\n")
      val data = lines.map(line => line.split(SQLConstant.REGEX_CUSTOM_SEPARATOR).map(word => word.trim).toSeq)
  ...
  override def buildScan(requiredColumns: Array[String]): RDD[Row] = {
    println("PrunedScan: buildScan called...") //TODO: when call this buildScan when call the one above
